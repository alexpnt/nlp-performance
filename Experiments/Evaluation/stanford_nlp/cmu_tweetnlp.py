...
def stanford_nlp_pos(finput):
	...
	for line in corpora:
		...
		proc = subprocess.Popen(["java","-cp",STANDORD_NLP+"*","-Xmx2g","edu.stanford.nlp.pipeline.StanfordCoreNLP","-annotators","tokenize,ssplit,pos,lemma,ner,parse","-file",STANFORD_OPENNPL_DIR+"input.txt","-outputDirectory",ROOT+"static/xml"],stdout=subprocess.PIPE,stdin=subprocess.PIPE)
