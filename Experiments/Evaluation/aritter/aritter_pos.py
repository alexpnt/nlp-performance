...
def aritter_pos(finput):
	...
	for line in corpora:
		...
		proc = subprocess.Popen(["python2",ARITTER_DIR+"python/ner/extractEntities2.py",'--pos'],stdout=subprocess.PIPE,stdin=subprocess.PIPE)
		...
		for i in xrange(len(ner)):
			ner[i]=(ner[i].split("/")[0],ner[i].split("/")[2].rstrip("\n"))
	...
	aritter_pos_results=open("aritter_pos_output.txt","w")
	for sentence in results:
		for token_pos in sentence:
			aritter_pos_results.write(token_pos[0]+" "+token_pos[1]+"\n")
		aritter_pos_results.write("\n")
	aritter_pos_results.close()

if __name__ == '__main__':

	parser = argparse.ArgumentParser(description='Aritter POS Task',epilog="POS with Aritter.\n Each input file is a collection of sentences.\nEach line has a sentence or a set of sentences")
